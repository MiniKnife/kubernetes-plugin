...
public class PodTemplateStepExecution extends AbstractStepExecutionImpl {
	...
	public boolean start() throws Exception {
		...
		// Let's generate a random name based on the user specified to make sure
		// that we don't have
		// issues with concurrent builds, or messing with pre-existing
		// configuration
		...
		newTemplate.setInheritFrom(!Strings.isNullOrEmpty(podTemplateAction.getParentTemplates())
				? podTemplateAction.getParentTemplates() : step.getInheritFrom());
	...
	private class PodTemplateCallback extends BodyExecutionCallback.TailCall {
		...
		protected void finished(StepContext context) throws Exception {
			...
			if (cloud instanceof KubernetesCloud) {
				LOGGER.log(Level.INFO, "Removing pod template {0} from cloud {1}",
						new Object[] { podTemplate.getName(), cloud.name });
				KubernetesCloud kubernetesCloud = (KubernetesCloud) cloud;
				kubernetesCloud.removeTemplate(podTemplate);

				if (podTemplate.getIdleMinutes() == 0) {
					LOGGER.log(Level.INFO, "Deleting pod {0} from cloud {1}",
							new Object[] { podTemplate.getName(), cloud.name });
